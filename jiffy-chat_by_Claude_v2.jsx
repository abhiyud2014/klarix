...
async function fixSQL(question, badSQL, errorMsg) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    method:"POST", headers:{"Content-Type":"application/json"},
    body: JSON.stringify({
      model:"claude-sonnet-4-20250514", max_tokens:600,
      system:`You are a SQL expert. A query for AlaSQL failed. Rewrite it using ONLY simple patterns.
...
Return ONLY JSON: {"sql":"SELECT ...","intent":"what this simplified query does","followups":["Q1?","Q2?","Q3?"]}`,
...
async function askLLM(question) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      model:"claude-sonnet-4-20250514", max_tokens:600,
      system:`You are a SQL expert writing queries for AlaSQL (an in-browser SQL engine with limitations).
...
- If ambiguous, make the most useful interpretation`,
...
async function exportPDF(pairs) {
  ...
  const drawHeader = (qNum, question) => {
    ...
    doc.text(`JIFFY · BRANDSCAPES WORLDWIDE · ${now} · Page ${pageNum}`, W-20, 14, {align:"right"});
  };
  ...
  doc.text("Jiffy Insights Report", 38, 168);
  ...
  doc.text(`Generated ${now}  ·  Model: claude-sonnet-4  ·  Powered by Jiffy AI`, 38, 238);
  ...
  doc.setTextColor(255,255,255);
  doc.text("J", 60, H-70, {align:"center"});

  // "JIFFY · AI DATA ANALYST" beside badge
  doc.setFont("helvetica","normal"); doc.setFontSize(10);
  doc.setTextColor(58,96,112);
  doc.setCharSpace(3);
  doc.text("JIFFY  ·  AI DATA ANALYST", 92, H-72);
  doc.setCharSpace(0);
  ...
  doc.text("JIFFY  ·  BRANDSCAPES WORLDWIDE  ·  AI-DRIVEN DATA INSIGHTS", W/2, H-8, {align:"center"});
  doc.setCharSpace(0);
  ...
  a.href = url;
  a.download = `jiffy-report-${Date.now()}.pdf`;
...
async function exportPPTX(pairs) {
  ...
  pptx.title = "Jiffy Insights Report";
  const W = 13.3, H = 7.5;
  const now = new Date().toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"});
  ...
  // Main title
  cover.addText("Jiffy Insights Report", { x:0.5, y:2.15, w:8.5, h:1.1, fontSize:48, bold:true, color:"FFFFFF", fontFace:"Calibri" });
  ...
  // Meta line
  cover.addText(`Generated ${now}  ·  Model: claude-sonnet-4  ·  Powered by Jiffy AI`, { x:0.5, y:3.95, w:8, h:0.35, fontSize:11, color:"4A6880", fontFace:"Calibri" });
  ...
  cover.addText("J", { x:0.5, y:5.65, w:0.7, h:0.7, fontSize:24, bold:true, color:"FFFFFF", align:"center", valign:"middle", fontFace:"Calibri" });
  cover.addText("JIFFY  ·  AI DATA ANALYST", { x:1.35, y:5.8, w:4, h:0.4, fontSize:11, color:"3A6070", charSpacing:3, fontFace:"Calibri", valign:"middle" });
  ...
  for (let idx = 0; idx < pairs.length; idx++) {
    ...
    try {
      ...
      sl.addText("JIFFY  ·  BRANDSCAPES WORLDWIDE  ·  AI-DRIVEN DATA INSIGHTS", {
  ...
  await pptx.writeFile({ fileName: `jiffy-insights-${Date.now()}.pptx` });
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    document.addEventListener("jiffy-followup", handler);
    return () => document.removeEventListener("jiffy-followup", handler);
  });
  ...
  const sendMessage = async (text) => {
    ...
    try {
      setActiveStep(1);
      const {sql, intent, answer, insight, followups, inputTokens, outputTokens} = await askLLM(q);
      ...
      let retried=false, retryTokens={in:0,out:0};
      try {
        result = executeSQL(sql);
      } catch(sqlErr) {
        // Auto-retry: ask LLM to simplify the failing SQL
        try {
          setActiveStep(1);
          const fix = await fixSQL(q, sqlErr.sql||sql, sqlErr.message);
          retryTokens = {in:fix.inputTokens, out:fix.outputTokens};
          const retryCost = calcCost(fix.inputTokens, fix.outputTokens);
      ...
      setActiveStep(4);
      setMessages(m=>[...m,{role:"assistant",type:"result",sql:finalSQL,intent:finalIntent,answer:finalAnswer,insight:finalInsight,result,cost,inputTokens,outputTokens,followups:finalFollowups,retried}]);
    } catch(e) {
        ...
        <aside style={{width:260,background:T.bgSide,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",flexShrink:0,overflowY:"auto",overflowX:"hidden",transition:"background .25s,border-color .25s"}}>
          ...
          <div style={{padding:"14px 14px 12px",borderBottom:`1px solid ${T.border}`,flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
              <div style={{width:36,height:36,background:`linear-gradient(135deg,${T.accent},${T.accent2})`,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:15,color:"#fff",boxShadow:`0 0 14px ${T.accentGlow},0 2px 8px rgba(0,0,0,.3)`,flexShrink:0}}>J</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            ...
            {p.followups.map((q,i)=>(
              <button key={i} className="fup-btn"
                onClick={()=>document.dispatchEvent(new CustomEvent("jiffy-followup",{detail:q,bubbles:true}))}
